package part3datamanipulation

import java.util.concurrent.Executors
import scala.concurrent.{ExecutionContext, Future}

object Writers {

  import cats.data.Writer
  //1 - define thm at the start
  val aWriter: Writer[List[String], Int] = Writer(List("Started something"), 45)
  val anIncreasedWriter = aWriter.map(_ + 1) //value increases logs stay the same
  val aLogsWriter = aWriter.mapWritten(_ :+ "something") //value stays the same, logs change
  val aWriterWithBoth = aWriter.bimap(_ :+ "something", _ + 1) //both value and logs change
  val aWriterWithBoth2 = aWriter.mapBoth((logs, value) =>(logs :+ "something", value + 1))


  //flatMap
  import cats.instances.vector._
  val writerA = Writer(Vector("Log A1", "Log A2"), 10)
  val writerB = Writer(Vector("Log B1"), 48)

  val compositeWriter = for {
    va <- writerA
    vb <- writerB
  } yield va + vb

  import cats.instances.list._ //an implicit for Monoid[List[Int]]
  //reset the logs
  val anEmptyWriter = aWriter.reset

  //3 - dump either the value or the logs

  val desiredValue = aWriter.value
  val logs = aWriter.written
  val (l, v) = aWriter.run

  //TODO 1: rewrite a function which "print" things with the writers
  def countAndSay(n: Int): Unit = {
    if (n <= 0) println("starting!")
    else{
      countAndSay(n-1)
      println(n)
    }
  }


  def countAndLog(n: Int): Writer[Vector[String], Int] = {
    if (n<= 0) Writer(Vector[String]("Starting"), 0)
//    else countAndLog(n-1).flatMap(_ => Writer(Vector(s"$n"), n))
    else countAndLog(n-1).bimap(a => a :+ s"$n", _ + n)
  }
  //benefit #1: we work with pure FP
  //TODO 2: rewrite this method with writers
  def naiveSum(n: Int): Int = {
    if (n <= 0) 0
    else {
      println(s"Now at $n")
      val lowerSum = naiveSum(n - 1)
      println(s"Computed sum ${n-1} = $lowerSum")
      lowerSum + n
    }
  }

  def naiveSumWithWriters(n: Int): Writer[Vector[String], Int] = {
    if (n <= 0) Writer(Vector[String](), 0)
    else for {
      _ <- Writer(Vector(s"Now at $n"), n)
      lowerSum <- naiveSumWithWriters(n-1)
      _ <- Writer(Vector(s"Computed sum ${n-1} = $lowerSum"), n)
    } yield lowerSum + n
  }

  //benefit #2: writers can keep logs separate from the multiple threads
  implicit val ex: ExecutionContext = ExecutionContext.fromExecutorService(Executors.newFixedThreadPool(4))
  def main(args: Array[String]): Unit = {
//    println(compositeWriter.run)
    //ex1
//    countAndLog(10).written.foreach(println)

    //ex2
//    naiveSumWithWriters(100).written.foreach(println)
    Future(naiveSum(100)).foreach(println)
    Future(naiveSum(100)).foreach(println)

    val sumFuture1= Future(naiveSumWithWriters(100))
    val sumFuture2= Future(naiveSumWithWriters(100))
    val logs1 = sumFuture1.map(_.written) //logs from thread 1
    val logs2 = sumFuture2.map(_.written) //logs from thread 2
  }

}
